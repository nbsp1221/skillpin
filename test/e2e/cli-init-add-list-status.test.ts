...
const home = makeCliHome("skillpin-cli-roundtrip");
...
describe("skillpin CLI init add list status", () => {
  it("init add list status JSON round trip", async () => {
    const init = await runSkillpin(["init", "--json"], home);
    const add = await runSkillpin(["add", "test/fixtures/skills/react-performance-review", "--json"], home);
    const list = await runSkillpin(["list", "--json"], home);
    const status = await runSkillpin(["status", "--json"], home);
