...
import { resolveSkillpinPaths, type ResolvePathsOptions } from "./paths.js";
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      skills.push({
        ...
        warnings_at_import: await readWarningsAtImport(join(paths.skillsDir, entry, ".skillpin.json")),
      });
    } catch (error: unknown) {
      invalid.push({
        managed_path: skillFile,
